...
def creerY(numCol):          # WAS creerListeColonne
    ...
    for i in range(1, nombreRecords):	    # WAS: 2 boucles, maintenant 1 / plus utiliser moyenne, mais valeur précédente si valable
        if fibListe[i][numCol] != '' and fibListe[i][numCol] != '0':        # si le champ n'est pas vide et ne contient pas de 0,
        ...
        colListe.append(fibListe[i][numCol])       # on passe un string, mais pas de problème: il est automatiquement converti en int ou float
    return colListe


# PROGRAMME PRINCIPAL
if __name__ == '__main__':                          # la façon officielle pour délimiter le programme principal
                                                    # permet aussi bien l'utilisation comme programme indépendant
                                                    # que import de ce fichier comme module dans un autre .py
    fibListe = creerFibTableau("jawbone.csv")       # la fonction retourne le tableau 2D
    nombreRecords = lireNombreRecords(fibListe)
    xListe = creerX()
    ...
    #print fibListe[0][13]           # m_distance
