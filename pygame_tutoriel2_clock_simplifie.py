...
while 1:                                        # crée une boucle infinie (1 est la même chose que True)
    ...
    # WAS: deplacer le rectangle, qui va contenir l'image de la balle (on ne peut pas déplacer l'image même)
    # deplacer l'image même
    ball = ball.move(speed)             # à chaque tour de boucle on appelle la méthode move (WAS: des rectangles)

    # WAS: si le rectangle dépasse les limites de l'écran on inverse sa direction
    # si l'image dépasse les limites de l'écran on inverse sa direction
    ...
    # WAS: screen.blit(ball, ballrect)         #  blit de l'image ball (source) dans ballrect (destination)
    screen.blit(ball)
    ...
    clock.tick(30)                      # max 30 frames par seconde (FPS)